...
void CEconItemSystem::ReloadAllSchemas()
{
    ...
    // Trigger a post-schema initialization to finalize the schema
    CUtlVector< CUtlString > vecErrors;
    bool bSuccess = m_itemSchema.BPostSchemaInit( &vecErrors );
...
#ifdef CLIENT_DLL
    // Fire events to update UI elements that depend on the item schema
    IGameEvent *schemaEvent = gameeventmanager->CreateEvent( "item_schema_initialized" );
    if ( schemaEvent )
    {
        gameeventmanager->FireEventClientSide( schemaEvent );
    }

    // Fire inventory_updated event to refresh loadout panels
    IGameEvent *inventoryEvent = gameeventmanager->CreateEvent( "inventory_updated" );
    if ( inventoryEvent )
    {
        gameeventmanager->FireEventClientSide( inventoryEvent );
    }
#endif
}
